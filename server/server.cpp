...
int main() {
  ...
  while (true) {
    ...
    int nev = kevent(kq, NULL, 0, events, 64, timeout_ptr);
    if (nev < 0) {
      if (errno == EINTR)
        continue;
      perror("kevent");
    ...
    for (int i = 0; i < nev; i++) {
      ...
      if (fd == listen_fd && filter == EVFILT_READ) {
        ...
        int connfd = handle_accept(listen_fd);
        if (connfd >= 0) {
          Conn *conn = g_data.fd2conn[connfd];
          update_events(connfd, conn, kq);
        }
      } else {
    ...
    // handle timers (idle connections)
    process_timers();
  }
  close(kq);
  return 0;
