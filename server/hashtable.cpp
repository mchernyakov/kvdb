...
static void hm_trigger_shrinking(HMap *hmap) {
  assert(hmap->older.tab == NULL);
  // (newer, older) <- (new_table, newer)
  hmap->older = hmap->newer;
  size_t new_size = (hmap->newer.mask + 1) / 2;
  if (new_size < 4)
    new_size = 4; // Avoid too small tables
  h_init(&hmap->newer, new_size);
  hmap->migrate_pos = 0;
}
